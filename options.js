...
async function generateAiPolish() {
  ...
  try {
    const port = chrome.runtime.connect({ name: 'ai-polish-stream' });
    currentAiPort = port;
    ...
    port.onMessage.addListener((msg) => {
      switch (msg.type) {
        ...
        case 'done':
          isStreamDone = true;
          aiResultCache.set(cacheKey, msg.result);
          renderAiPolishResult(msg.result);
          showResultButtons();
          isAiGenerating = false;
          setAiLoading(false);
    ...
    port.onDisconnect.addListener(() => {
      if (!isStreamDone && isAiGenerating) {
        renderAiPolishError({ errorCode: 'network_error', error: '连接中断' });
        isAiGenerating = false;
        setAiLoading(false);
        streamContainer.style.display = 'none';
        currentAiPort = null;
...
function clearAiPolishResult() {
  ...
  document.getElementById('ai-stream-container').style.display = 'none';
  document.getElementById('ai-stream-content').textContent = '';
  updateAiActionButtons();
}
...
async function renderApiConfigs() {
  ...
  configs.forEach(config => {
    ...
    wrapper.className = 'api-config-item';
